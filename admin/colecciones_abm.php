<?  include('../includes/loader.inc.php'); 
	...
	$row = $db->get_row("SELECT * FROM ".$config['modulo']." WHERE id_coleccion = '".$_GET['id']."'");
					...
					<?#	foreach ($data['update'] as $id => $update): ?>
						...
						<ul class="list">
							...
							<li>
								...
								<p><input type="text" id="form-nombre" name="data[<?=$config['modulo']?>][orden]" value="<?=$row['orden']?>"/> (se ordena de menor a mayor)</p>
							</li>
					
							<!-- text -->
							<li>
								<p><label for="form-descripcion">Descripcion</label></p>
								<p><input type="text" id="form-descripcion" name="data[<?=$config['modulo']?>][descripcion]" value="<?=$row['descripcion']?>"/></p>
							</li>
