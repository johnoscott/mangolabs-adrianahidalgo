...
/*
		define("EZSQL_DB_USER", "avantrip");        // <-- mysql db user
    define("EZSQL_DB_PASSWORD", "MjKhi9BU");    // <-- mysql db password 
    define("EZSQL_DB_NAME", "avantrip");        // <-- mysql db pname
    define("EZSQL_DB_AGENCIA_NAME", "avantrip_agencia");
    define("EZSQL_DB_HOST", "192.168.0.57");        // <-- mysql server host
*/

	// ==================================================================
	//	ezSQL Constants
	define("EZSQL_VERSION","1.13");
	...
	class ez_sql {
		...
		function ez_sql($dbuser="", $dbpassword="", $dbname="", $dbhost="")
		{
			if ($dbuser == "") $dbuser = EZSQL_DB_USER;
			if ($dbpassword == "") $dbpassword = EZSQL_DB_PASSWORD;
			if ($dbname == "") $dbname = EZSQL_DB_NAME;
			if ($dbhost == "") $dbhost = EZSQL_DB_HOST;
		...
		// ==================================================================
		//	OMILEN: Agregado por Juanma

		function queryWithMultipleLines($query)
		{
			$linesArray = split("\n",$query);
			foreach($linesArray as $line){
				if($line=="") continue;
				mysql_query($line,$this->dbh);
			}
			if ( mysql_error() )
			{
...
/*
// Debug
echo '<p style="color: red; font-weight: bold"><br>' . $query . '<br></p>';
...
/*			
   ...
   echo "La consulta tardo " . $totaltime . " segundos"; 
...
*/	
			...
			// Si esta activado el debug, lo ejecuto
			if ($this->debug)
				$this->debug();
		...
		function debug()
		{
			...
			echo "[<font color=000000><b>".htmlentities($this->last_query)."</b></font>]</font><p>";
		...
		function insert ($tabla, $data) {
			...
			if($campos = $this->get_results("SHOW fields FROM $tabla", 'OBJECT'))
				foreach ($campos as $c) {
					if (isset($data[$c->Field])) {
						$fields .= $c->Field.', ';
						$values .= "'".addslashes($data[$c->Field])."', ";
					}
				}
			...
			return $this->insert_id = mysql_insert_id($this->dbh);
		}

		function update ($tabla, $data, $where=null) {
			$query = "UPDATE $tabla SET ";

			if($campos = $this->get_results("SHOW fields FROM $tabla", 'OBJECT'))
				foreach ($campos as $c)
					if (isset($data[$c->Field]) && ($c->Field != "id"))
						$query .= $c->Field." = '".addslashes($data[$c->Field])."', ";

			$query = substr($query, 0, strlen($query)-2);
			$query .= " WHERE ".(($where)? preg_replace('/^where/i', '', $where) : $campos[0]->Field." = ".$data[$campos[0]->Field]);
			$this->query($query);

			return true;
		}

		function replace ($tabla, $data) {
			$query = "REPLACE INTO $tabla";
...
//$db = ez_sql::getInstance(EZSQL_DB_USER, EZSQL_DB_PASSWORD, EZSQL_DB_NAME, EZSQL_DB_HOST);
?>
